...
@Component
public class BidOrderExchangeImpl implements OrderExchange {
    ...
    public Trade doExchange(Order source, Order target) {
        ...
        if (source.getVolume().equals(BigDecimal.ZERO)) {
            OrderBook.BID_ORDER_BUCKET.remove(source);
        }

        if (target.getVolume().equals(BigDecimal.ZERO)) {
            OrderBook.ASK_ORDER_BUCKET.remove(target);
        }
    ...
    public List<Trade> create(Order order) {
        ...
        if (OrderBook.BID_ORDER_BUCKET.contains(order)) {
            return Collections.emptyList();
        }

        List<Trade> trades = new ArrayList<>();
        for (Order targetOrder : OrderBook.ASK_ORDER_BUCKET) {
        ...
        if (order.getVolume().equals(BigDecimal.ZERO)) {
            OrderBook.BID_ORDER_BUCKET.add(order);
        }
    ...
    public void cancel(Order order) {
        OrderBook.BID_ORDER_BUCKET.remove(order);
    }

    @Override
    public void cancel(String id) {
        Order order = OrderBook.BID_ORDER_BUCKET.stream().filter(e -> e.getId().equals(id))
